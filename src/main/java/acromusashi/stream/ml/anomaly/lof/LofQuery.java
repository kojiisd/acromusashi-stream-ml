...
public class LofQuery extends BaseQueryFunction<MapState<LofDataSet>, String>
{
    ...
    public List<String> batchRetrieve(MapState<LofDataSet> state, List<TridentTuple> tuples)
    {
        ...
        // Get model
        List<LofDataSet> dataSets = state.multiGet(
                Arrays.asList(Arrays.asList((Object) this.baseStateName)));
        ...
        for (TridentTuple targetTuple : tuples)
        {
            ...
            {
                lofScore = LofCalculator.calculateLofNoIntermediate(this.kn, recievedPoint,
                        dataSet);
            }
            ...
            {
                logger.warn(
                        "Received data is invalid. skip this data. ReceivedData=" + recievedPoint,
                        ex);
            }
